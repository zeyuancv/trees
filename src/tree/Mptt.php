...
class Mptt {
     ...
     *  @return void
     */
    public function __construct(&$link, $table_name = 'cherry', $id_column = 'ID', $title_column = 'Title', $left_column = 'Lft', $right_column = 'Rgt', $parent_column = 'Parent') {
     ...
     */
    public function add($parent, $title, $description='',$addition=[],$position = false) {
        ...
        if (
			...
			$additionKeyStr = '';
			$additionValueStr = '';			
			foreach($addition as $addKey=>$addVal){
				if(!isset($this->properties[$addKey])){
					$this->properties[$addKey] = ucfirst($addKey);
				}
				$additionKeyArr[] = '`' . $addKey . '`';
				$additionValueArr[] = '"' . mysqli_real_escape_string($this->link, $addVal) . '"';
			}

			if(count($additionKeyArr)>0){
				$additionKeyStr = ','.implode(',',$additionKeyArr);
			}
			
			if(count($additionValueArr)>0){
				$additionValueStr = ','.implode(',',$additionValueArr);
			}
            ...
            mysqli_query($this->link, '
                INSERT INTO
                    ...
                    (
						...
						`' . $this->properties['description_column'] . '`,
                ...
                VALUES
                    (
						...
						"' . mysqli_real_escape_string($this->link, $description) . '",
                        ' . ($boundary + 1) . ',
                        ' . ($boundary + 2) . ',
                        ' . $parent . '
						'.$additionValueStr.'
     ...
     */
  function copyNode($source, $target, $position = false) {
        ...
        if (
            ...
            $target_children = $this->get_descendants($target,false);
            ...
            foreach ($sources as $id => $properties) {
                ...
                foreach ($sources as $key => $value)
                    ...
                    if ($value[$this->properties['parent_column']] == $sources[$id][$this->properties['id_column']])
                ...
                $sources[$id][$this->properties['id_column']] = $node_id;
            }
     ...
     *  @since  2.2.5
     */
    public function update($node, $title,$description='',$addition=[]) {
        ...
        if (isset($this->lookup[$node])) {
			...
			foreach($addition as $addKey=>$addVal){
				...
				$additionValueArr[] = '`' . $this->properties[$addKey] . '` = "' . $addVal . '"';
			}
            ...
            mysqli_query($this->link, '
                ...
                SET
                    `' . $this->properties['title_column'] . '` = "' . $title . '",
					`' . $this->properties['description_column'] . '` = "' . $description . '"
					'.$additionValueStr.'
                WHERE
